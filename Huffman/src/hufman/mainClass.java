...
public class mainClass {
	...
	public static void createMinHeapNew( int[] arr , int size  ) {
		for( int i = 0 ; i < size/2 ; i++ ) {
			...
			if( rcp < size &&  arr[ rcp ] < arr[ i ]  ) { switchElment( arr , rcp , i ); }
			if( lcp < size &&  arr[ lcp ] < arr[ i ]  ) { switchElment( arr , lcp , i ); }
		}
	}
	...
	public static void maxHeapify( int[] arr , int size , int index ){
		...
		if( leftNode < size && arr[l] < arr[ leftNode ] ) { l = leftNode; }
		if( rightNode < size && arr[l] < arr[ rightNode ] ) { l = rightNode; }
		if( l != index ) {
			int temp = arr[ index ] ;
			arr[ index ] = arr[ l ] ;
			arr[ l ] = temp ;
